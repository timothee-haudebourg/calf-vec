...
#[test]
fn test_from_str() {
	let owned: Option<CalfString<N>> = "string".parse().ok();
	assert_eq!(owned.as_ref().map(|s| &**s), Some("string"));
}

#[test]
fn test_from_cow_str() {
	assert_eq!(CalfString::<N>::from(Cow::Borrowed("string")), "string");
	assert_eq!(CalfString::<N>::from(Cow::Owned(String::from("string"))), "string");
}

#[test]
fn test_from_utf8() {
	let xs = b"hello".to_vec();
	assert_eq!(CalfString::<N>::from_utf8(xs).unwrap(), CalfString::<N>::from("hello"));

	let xs = "ศไทย中华Việt Nam".as_bytes().to_vec();
	assert_eq!(CalfString::<N>::from_utf8(xs).unwrap(), CalfString::<N>::from("ศไทย中华Việt Nam"));

	let xs = b"hello\xFF".to_vec();
	let err = CalfString::<N>::from_utf8(xs).unwrap_err();
